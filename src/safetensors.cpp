...
std::pair<std::uint64_t, std::uint64_t> parse_offsets(const std::string& body) {
    ...
    const auto first = std::stoull(body.substr(begin, comma - begin));
    const auto second = std::stoull(body.substr(comma + 1, end - comma - 1));
    return {first, second};
}
...
void Reader::open() {
    ...
    if (end < std::streamoff(8)) {
        throw std::runtime_error("safetensors file is smaller than header prefix");
    }
...
void Reader::parse_header(const std::string& header) {
    ...
    while (true) {
        ...
        const auto object_begin = header.find('{', pos);
        if (object_begin == std::string::npos) throw std::runtime_error("malformed tensor object");
        const auto object_end = header.find('}', object_begin);
        if (object_end == std::string::npos) throw std::runtime_error("malformed tensor object");
        ...
        if (elements > std::numeric_limits<std::uint64_t>::max() / dtype_size(info.dtype)) {
